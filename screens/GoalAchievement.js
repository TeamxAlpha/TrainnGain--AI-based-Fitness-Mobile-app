...
const GoalAchievement = () => {
  ...
  const setNewGoal = async () => {
    try {
      const response = await axios.post(`http://172.16.21.47:5001/set-goal/${userId}`, { goal }); //Zohaib's 192.168.137.1, Mahdi's 192.168.100.8
  ...
  const checkGoal = async () => {
    try {
      const response = await axios.get(`http://172.16.21.47:5001/check-goal/${userId}`); //Zohaib's 192.168.137.1, Mahdi's 192.168.100.8
